...
class HyperComponent extends HTMLElement {
  ...
  sendTransformationsOther() {
    ...
    self.payloadObj.transformations = self.transformationIDNoActive;
    self.sendPayload(self.showHyperedCarousel.bind(self));
    self.setPayloadWActiveMedia(self.mediaGalleryMediaActive);
  }
...
class BirthstoneOptions extends HTMLElement {
  ...
  addSelectedAccessory(label) {
    ...
    self.showAccessoryEngraving(self.selectedAccessoryArr);
  ...
  showAccessoryEngraving(arr) {
    var wrappers = document.querySelectorAll('.birthstone-engraving');
    for (var i = 0; i < wrappers.length; i++) {
      var wrap = wrappers[i],
          engraving_label_accessory = wrap.querySelector('.option-name');
      if (arr[i])
      {
        var accessory_title = arr[i].querySelector('.so-birthstone-input-selected').getAttribute('data-option-value-short');
        wrap.classList.remove('so-hidden');
        if(wrap.querySelector('.so-input-birthstone').hasAttribute('required') || wrap.querySelector('.so-input-birthstone').hasAttribute('data-required')) {
          wrap.querySelector('.so-input-birthstone').setAttribute('required', true);
        }
        engraving_label_accessory.textContent = ' ' + accessory_title;
        let engDataName = wrap.querySelector('.so-input-birthstone').getAttribute('data-name');
        wrap.querySelector('.so-input-birthstone').setAttribute('name', engDataName);
      }
      else
      {
        wrap.classList.add('so-hidden');
        if(wrap.querySelector('.so-input-birthstone').hasAttribute('required')) {
          wrap.querySelector('.so-input-birthstone').removeAttribute('required');
          wrap.querySelector('.so-input-birthstone').setAttribute('data-required', 1);
        }
        wrap.querySelector('.so-input-birthstone').value = '';
        wrap.querySelector('.so-input-birthstone').removeAttribute('name');
      }
    }
  }
...
class AccessoryOptions extends HTMLElement {
  ...
  showAccessoryEngraving(arr) {
    var wrappers = document.querySelectorAll('.accessory-engraving');
    for (var i = 0; i < wrappers.length; i++) {
      var wrap = wrappers[i],
          engraving_label_accessory = wrap.querySelector('.option-name');
      if (arr[i])
      {
        var accessory_title = arr[i].querySelector('.so-accessory-input-selected').getAttribute('data-option-value-short');
        wrap.classList.remove('so-hidden');
        if(wrap.querySelector('.so-input-accessory').hasAttribute('required') || wrap.querySelector('.so-input-accessory').hasAttribute('data-required')) {
          wrap.querySelector('.so-input-accessory').setAttribute('required', true);
        }
        engraving_label_accessory.textContent = ' ' + accessory_title;
        let engDataName = wrap.querySelector('.so-input-accessory').getAttribute('data-name');
        wrap.querySelector('.so-input-accessory').setAttribute('name', engDataName);
      }
      else
      {
        wrap.classList.add('so-hidden');
        if(wrap.querySelector('.so-input-accessory').hasAttribute('required')) {
          wrap.querySelector('.so-input-accessory').removeAttribute('required');
          wrap.querySelector('.so-input-accessory').setAttribute('data-required', 1);
        }
        wrap.querySelector('.so-input-accessory').value = '';
        wrap.querySelector('.so-input-accessory').removeAttribute('name');
      }
    }
  }
  ...
  changeAccessoryMetal() {
    var self = this;
    var accessoryImgs = Array.from(self.querySelectorAll('.so-custom-field-accessory-img'));
